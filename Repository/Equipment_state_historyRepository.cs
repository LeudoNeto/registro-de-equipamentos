...
namespace equipment_state_history.Repository
{
    public class Equipment_state_historyRepository : IEquipment_state_historyRepository
    {
        ...
        public async Task<Equipment_state_history> BuscaEquipment_state_history(Guid equipment_id)
        {
            return await _context.Equipment_state_historys.Where(x => (x.Equipment_id == equipment_id)).OrderBy(x => x.Date).LastOrDefaultAsync();
        }
